...
import md5 from "js-md5"; // 导入MD5加密算法

export const Administrator = () => {
    const role = store.getState().userData.role; // 获取当前角色的身份
        ...
        role === 0 ?
            {
                ...
                render: (text, record, index) => { // text：当前行的值，record：当前行的数据，index：当前行索引
                    ...
                    return (
                        <Space size={"middle"}>
                            {
                                record.role === 0 ? (<Button type={"link"} icon={<EditOutlined />} disabled>编辑</Button>) :
                                    (<Button type={"link"} icon={<EditOutlined />} onClick={() => showModifyModal(record)}>编辑</Button>)
                            }
                            {
                                record.role === 0 ? (<Button type={"link"} icon={<DeleteOutlined />} disabled >删除</Button>) :
                                    (<Button type={"link"} icon={<DeleteOutlined />} danger onClick={() => showDeletConfirm(record.id)}>删除</Button>)
                            }
                        </Space>
                    )
                }
    ...
    const addAdministrator = async () => {
        await addRef.current.validateFields()
            .then(value => {
                ...
                axios.post(`${HOST}:${PORT}/admins/register`, { user })
                    .then(result => {
    ...
    const deletePerson = (id) => { // 删除某个人
        // console.log(typeof id) // id是获取到的索引，number类型
